...
def llm_analyze_review(review_text, original_rating, primary_job):
    ...
    2. Extract a standalone professionalism score (cleanliness, punctuality).
    3. Discover new, unlisted skills the worker performed during the job.
    """
    if not GROQ_API_KEY: return original_rating, original_rating, []
...
Task 1 (Sentiment): Determine 'adjusted_rating' (1.0 - 5.0). If they gave 5 stars but complained (e.g. "left a mess"), penalize it. If they gave 3 stars but the text is glowing, boost it.
  ...
  "hidden_skills": [
    ...
    try:
        ...
        if match:
            ...
            adj_rating = float(data.get("adjusted_rating", original_rating))
            prof_score = float(data.get("professionalism", original_rating))
            hidden_skills = data.get("hidden_skills", [])
            return adj_rating, prof_score, hidden_skills
